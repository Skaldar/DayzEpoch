...
BankDialogWithdrawAmount = {
	...
	if (!isNull ZSC_CurrentStorage) then {
		...
		format ["Du hast %1 %2 aus %3 genommen",[_amount] call BIS_fnc_numberText,CurrencyName,_displayName] call dayz_rollingMessages;
	} else {
...
BankDialogDepositAmount = {
	...
	if ((_bank + _amount) > _maxCap) then {
		format ["Du kannst höchstens %1 %2 im %3 lagern.",[_maxCap] call BIS_fnc_numberText,CurrencyName,_displayName] call dayz_rollingMessages;
	} else {
		player setVariable[Z_MoneyVariable,(_wealth - _amount),true];
		ZSC_CurrentStorage setVariable[Z_MoneyVariable,(_bank + _amount),true];
		format ["Du hast %1 %2 in den %3 gelegt.",[_amount] call BIS_fnc_numberText,CurrencyName,_displayName] call dayz_rollingMessages;
	};
...
GivePlayerAmount = {
	...
	if ((_amount < 1) or {_amount > _wealth}) exitWith {"So viel Geld hast du gar nicht" call dayz_rollingMessages;};

	if (!(isPlayer ZSC_GiveMoneyTarget)) exitWith {"Du kannst nur einem Spieler Geld geben" call dayz_rollingMessages;};

	if (_isBusy) exitWith {format ["%1 zählt gerade sein Geld, versuche es gleich nochmal",name ZSC_GiveMoneyTarget] call dayz_rollingMessages;};
	...
	format ["Du hast %1 %2 an %3 übergeben.",name ZSC_GiveMoneyTarget,[_amount] call BIS_fnc_numberText,CurrencyName] call dayz_rollingMessages;
};
