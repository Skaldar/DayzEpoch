	...
	*/
	...
	if (_isMan && {!_isAlive} && {!(_cursorTarget isKindOf "Animal")} && {player distance _cursorTarget < 5}) then {
		if (s_player_checkWallet < 0) then {
			s_player_checkWallet = player addAction ["Taschen durchsuchen","scripts\zsc\checkWallet.sqf",_cursorTarget,0,false,true];
		};
	} else {
...
if ((_typeOfCursorTarget in DZE_MoneyStorageClasses || _isVehicle) && {!_isMan} && {!locked _cursorTarget} && {_isAlive} && !(_typeOfCursorTarget in DZE_LockedStorage) && {player distance _cursorTarget < 5}) then {
	if (s_bank_dialog < 0) then {
			s_bank_dialog = player addAction ["Geldspeicher","scripts\zsc\bankDialog.sqf",_cursorTarget,1,true,true];
		};
	} else {
	...
	if (_isAlive && {_typeOfCursorTarget in AllPlayers} && {isPlayer _cursorTarget}) then {
		if (s_givemoney_dialog < 0) then {
			s_givemoney_dialog = player addAction [format ["%1 mit %2 handeln",CurrencyName,name _cursorTarget],"scripts\zsc\givePlayer.sqf",_cursorTarget,3,true,true];
		};
	} else {
		...
		//Test
	if (__typeOfCursorTarget in ZSC_bankTraders && {player distance _cursorTarget < 5}) then {
				
			s_player_atm_dialog = player addAction ["Banking","scripts\zsc\atmDialog.sqf",_cursorTarget,0,false,true];
		
	} else {
		player removeAction s_player_atm_dialog;
		s_player_atm_dialog = -1;
	};
	//End Test
