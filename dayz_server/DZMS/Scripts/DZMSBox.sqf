...
_money = ["ItemTopaz","ItemObsidian","ItemSapphire"];
_Treasure = ["ItemTopaz","ItemObsidian","ItemSapphire","ItemAmethyst","ItemEmerald","ItemCitrine","ItemRuby"];
_TreasureFail = ["ItemGoldBar"];

///////////////////////////////////////////////////////////Treasure hunt by brw1990
if (_type == "Treasure") then {
    // load treasure
	_jackpot = (round(random 2));
	if (_jackpot == 1) then {
    _scount = count _Treasure;
	diag_log format["[DZMS]: Jackpot"];
	...
	{		
	_scount = count _TreasureFail;
	diag_log format["[DZMS]: Trostpreis"];
	_sSelect = floor(random _sCount);
    _item = _TreasureFail select _sSelect;
	_crate addMagazineCargoGlobal [_item,(round(random 5))];
	};	
};
